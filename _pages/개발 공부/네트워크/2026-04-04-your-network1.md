...
1. ping(Packet INternet Groper) : 네트워크의 상태를 확인하려는 대상 노드를 향해 일정 크기의 패킷을 전송하는 명령어
...
2. netstat : 접속되어 있는 서비스들의 네트워크 상태를 표시하는데 사용

**netstat**를 사용하면 네트워크 접속 상태, 라우팅 테이블, 네트워크 프로토콜 등의 리스트를 보여준다.

또한, 서비스의 포트가 열려있는지 확인할 때 사용하기도 한다.

3. nslookup : DNS에 관련된 내용을 확인하기 위해 쓰는 명령어

이는 특정 도메인에 매핑된 IP를 확인하기 위해 사용한다.

4. tracert(traceroute) : 목적지 노드까지 네트워크 경로를 확인할 때 사용하는 명령어
